...
int main()
{
	...
	while (Cas<cas)
	{
		...
		while (cnta<5)
		{
			...
			M1-=a[nowa];
			ans[t++].push_back((node){0,nowa,++Cnta,++Red[nowa]});
			nowa=Nexta[nowa]; 
		}
		ans[t++].push_back((node){0,5,0,0});
		...
		while (cnta<5)
		{
			...
			M1-=a[nowa];
			ans[t++].push_back((node){1,nowa,++Cnta,++Blue[nowa]});
			nowa = Nextb[nowa];
		}
		ans[t++].push_back((node){1,5,0,0});
		for (int i=0;i<=M;i++)
